...
public class SlottedBlock
{
    ...
    */
    ...
    {
        int slot_cnt = rid.slot_cnt;
        // Check if RID is actually in the block. If not, return false
        if (slot_cnt >= intBuffer.get(3)) {
            return false;
        }
        int r_size_index = 6 + (slot_cnt*2);
        if (intBuffer.get(r_size_index) == -1) {
            return false;
        }

        // Check if it is the end of slot array
        if (slot_cnt == intBuffer.get(3)-1) {
            int new_slot_cnt = intBuffer.get(3)-1;
            intBuffer.put(3,new_slot_cnt);
        }

        // Else:
        // 1) RECORDS: delete the records, set to NULL
        int r_pos = intBuffer.get(5+(2*slot_cnt));
        int r_size = intBuffer.get(5+(2*slot_cnt+1));
        int free_Pointer = intBuffer.get(4);
        // shift records to cover the hole
        for (int i= r_pos-r_size; i> intBuffer.get(4); i--) {
            data[i+r_size] = data[i];
        }

        // 2) SLOT ARRAY: update end of freespace
        free_Pointer = free_Pointer+r_size;
        intBuffer.put(4, free_Pointer);

        // get the length of this record, offset that from positions from all the records (+ length)
        int newPos;
        for (int i = r_size+1; i < (intBuffer.get(3)*2+5); i++) {
            newPos = intBuffer.get(i) + r_size;
            intBuffer.put(i, newPos);
        }
    ...
    public RID firstRecord()
    {
        int slot_cnt = intBuffer.get(3);
        int count = 0;
        for (int i = 5; i < 5 + 2*(slot_cnt); i+=2) {
            if (intBuffer.get(i+1) != -1) {
                RID RIDToRtn = new RID(this.getBlockId(), count);
                return RIDToRtn;
            }
            count++;
        }
    ...
    */
    ...
    {
        int curRBID = curRid.blockId;
        int curRSID = curRid.slotNum;
        if (curRBID == INVALID_BLOCK) {
            throw new BadBlockIdException();
        }
        if (curRSID == INVALID_BLOCK) {
            throw new BadSlotIdException();
        }
        int cnt = curRSID+1;
        for (int i = 5 + 2*(curRSID)+2;i < 5+(intBuffer.get(3)*2)-1; i+=2) {
            if (intBuffer.get(i+1) != -1) {
                RID r_record = new RID(this.getBlockId(), cnt);
                return r_record;
            }
            cnt++;
        }
